# This code can give error in a different machine if sepolia id  and ethereum are not defined properly and there's not sufficient balance present.
...
web3 = Web3(Web3.HTTPProvider('https://sepolia.infura.io/v3/ca47fd6231534e92aba48d58bc2fb0c1'))  # Connect to your Ethereum node
...
def generate_transaction_id(sender_address):
    ...
    # Retrieve the current nonce value for the sender's address
    current_nonce = web3.eth.get_transaction_count(sender_address)

    # Use a nonce (e.g., a random number) to ensure uniqueness
    nonce = random.randrange(1000,100000)  # You should use a better nonce generation method
    nonce = current_nonce + 1  # Increment the nonce to ensure it's higher than the current value
...
def rent_car_ui():
    ...
    def rent_car():
        ...
        if car:
            ...
            sender_address = "0x29F0752a40978763FE4E717a56eEF7Dba9216474"  # Placeholder for user's Ethereum address
            receiver_address = "0x7324b423E80F1856abf1C0CbA1F323056fC9CF25"  # Placeholder for rental service's Ethereum address
            amount = float(price)
